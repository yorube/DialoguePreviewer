...
(function (global) {
  ...
  // {locale}, ID, Script, NodeTitle, Notes, ReviewStatus) by re-running
  // YarnConverter.buildSO over every loaded en-US project — same code path
  // Unity v2 uses, so the file is import-ready. Trailing Script / NodeTitle
  ...
  function buildSyntheticSource(targetLocale, ctx) {
    ...
    const headers = ['Type', 'Gender', 'CharacterName', 'en-US', targetLocale, 'ID', 'Script', 'NodeTitle', 'Notes', 'ReviewStatus'];
    const rows = [];
    const charCtx = {
  ...
  // with Script / NodeTitle / Notes / ReviewStatus columns so notes and
  ...
  function augmentSourceForExport(source, ctx) {
    ...
    const scriptCol = ensureCol('Script');
    ...
    // First pass: ensure every row has Script + NodeTitle if we can derive them.
    ...
    for (let i = 0; i < out.rows.length; i++) {
      ...
      if (!row[scriptCol]) row[scriptCol] = meta.group;
  ...
  function restoreNotesFromSource(source, ctx) {
    ...
    const scriptCol = headerIdx('Script');
    const nodeCol   = headerIdx('NodeTitle');

    // Reverse map: guid → group, used when the row has only an UID (no Script
    // column). Skipped if guids data isn't loaded.
    const guidToGroup = {};
    if (ctx.guids && ctx.getAllGroups && ctx.getEntry) {
      ...
      for (const [fn, g] of Object.entries(ctx.guids)) {
        if (filenameToGroup[fn]) guidToGroup[g] = filenameToGroup[fn];
      }
    }
    ...
    for (const row of source.rows) {
      ...
      let group = scriptCol !== -1 ? (row[scriptCol] || '').toString().trim() : '';
      let nodeTitle = nodeCol !== -1 ? (row[nodeCol] || '').toString().trim() : '';
      if ((!group || !nodeTitle) && idCol != null) {
        const uid = (row[idCol] || '').toString().trim();
        const m = uid.match(/^(.+)-(\d+)-\d+$/);
        if (m) {
          const inferredGroup = guidToGroup[m[1]];
          if (inferredGroup) {
            if (!group) group = inferredGroup;
            if (!nodeTitle) {
              const en = ctx.getEntry(inferredGroup, 'en-US');
